...
本仓库是[禅道（ZenTao）](https://www.zentao.net/)开源版**管理后台后端服务**的重写实现，
...
- 领域模型：产品 / 需求 / 项目 / 执行 / 任务 / Bug / 用例 / 文档等
- API 契约：`contract/openapi.yaml` 的接口划分与字段语义
- 文案与术语：`backend/src/main/resources/lang/*.json`
- 菜单树、权限码与角色设计、初始化种子数据

本仓库新写的内容：

- Java 实现代码（Spring Boot / MyBatis-Flex / Flyway）、测试、构建与部署配置
- 工程门禁与契约校验脚本见姊妹仓 [xiaoya-frontend](https://github.com/a414003183/xiaoya-frontend) 的 `tools/`
...
（例如 Java 包名 `net.zentao`、登录页标题），不表示获得商标授权，也不表示与上游存在隶属关系。
